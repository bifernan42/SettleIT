...
- **`analytics` module** — `GET /analytics/summary` (read-only) aggregates PaymentRequests joined to their `flowNode.flow`. Returns `totals` (recovered €, due €, fulfillmentRate), `byFlow` (per-flow recovered/due/fulfillmentRate, sorted by recovered), and `byChannel` (per-channel recovered, fulfillmentRate, deliveryFailureRate). Channel is normalized from `flowNode.settings` — handles both the seed shape `{ channel: 'email' }` and the flow-editor shape `{ actionType: 'EMAIL' }` → canonical `EMAIL|SMS|WHATSAPP|COURRIER`. `fulfillmentRate` is computed over *delivered* reminders (excludes DELIVERY_FAILED).
